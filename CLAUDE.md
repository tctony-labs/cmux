...
- **All user-facing strings must be localized.** Use `String(localized: "key.name", defaultValue: "English text")` for every string shown in the UI (labels, buttons, menus, dialogs, tooltips, error messages). Keys go in `Resources/Localizable.xcstrings` with translations for the two required languages in this fork: English (`en`) and Simplified Chinese (`zh-Hans`). Never use bare string literals in SwiftUI `Text()`, `Button()`, alert titles, etc.
- **Localization audit is required for every user-facing change.** Before finishing a task that changes UI, Settings rows, menus, shortcut metadata, schema/config text, docs, command/help text, alerts, or tooltips, enumerate the changed user-facing surfaces and verify each one has entries for English and Simplified Chinese. `defaultValue`, English fallback text, schema descriptions, or copied English strings do not count as localization. For Swift/AppKit strings, update `Resources/Localizable.xcstrings`; for localized web/docs content, update `web/messages/en.json` and `web/messages/zh-CN.json` and any localized data structures that carry inline translations. Parse touched localization files, compare changed message keys across locales, and use `rg` over changed Swift/TS/TSX/docs files for newly introduced bare English. The final handoff must state what localization audit was performed or explicitly say what could not be verified.
- **tctony localization scope:** Only English and Simplified Chinese are required. This overrides broader language requirements in `cmux-localization` and its references; existing translations for other languages may remain, but need not be updated. Keep `CHANGELOG.md` in English without duplicate translated entries. Agent instructions are not product UI and do not require parallel translations.
...
- `cmux-localization`：修改界面文字等用户可见文案时，仅要求英文和简体中文，以本文件的本地化范围为准。
